...
(function (FoodParent) {
    ...
    var PostNoteView = (function (_super) {
        ...
        PostNoteView.prototype.renderNoteInfo = function () {
            ...
            self.$('.input-amount').html(htmlDecode(self._note.getAmount()));
            self.$('.input-amount').on('click', function (event) {
                $(this).replaceWith("<input type='text' class='input-amount form-control'>" + self._note.getAmount() + "</textarea>");
                self.$('.input-amount').focus();
                self.$('.input-amount').on('focusout', function (event) {
                    console.log(self.$('.input-amount').val());
                    var comment = self.$('.input-amount').val();
                    self._note.setAmount(comment);
                    self.renderNoteInfo();
                });
            });
