var AdoptTreesCell = Backgrid.Cell.extend({
    ...
    render: function () {
        var trees: FoodParent.Trees = new FoodParent.Trees(FoodParent.Model.getInstance().getTrees().filterById(this.model.get('trees')));
        var data = {
            trees: trees,
        }
...
var AdoptAddCell = Backgrid.Cell.extend({
    ...
    AddAdoption: function (e) {
        ...
        var person: FoodParent.Person = FoodParent.Model.getInstance().getPersons().findWhere({ id: this.model.get('tempid') });
        var tree: FoodParent.Tree = FoodParent.View.getInstance().getPopupView().getCurrentTree();

        var r = confirm("Do you want assign tree '" + tree.getName() + "' to '" + person.getName() +"'");
        if (r == true) {
            e.preventDefault();
            var adopt: FoodParent.Adopt = new FoodParent.Adopt({
                tree: tree.getId(),
                owner: person.getId(),
            });
